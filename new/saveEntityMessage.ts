...
import { EntityFormCategory, ServerEntityCategory } from "../../enums/entityCategory.enum";

export type SaveEntityCategory = ServerEntityCategory;
export type SaveEntityType = "LINE" | "ELLIPSE" | "SECTOR" | "POLYGON";

export const toServerEntityCategory = (category: string): SaveEntityCategory => {
  const normalized = String(category || "").trim().toUpperCase().replace(/\s+/g, "_");
  if (normalized === "WCO_FREE") return ServerEntityCategory.WCO_FREE;
  if (normalized === "WCO_HOLD") return ServerEntityCategory.WCO_HOLD;
  if (normalized === "FIZ") return ServerEntityCategory.FIZ;
  return ServerEntityCategory.FREE;
};

export const toLocalEntityCategory = (category: string): string => {
  const normalized = String(category || "").trim().toUpperCase().replace(/\s+/g, "_");
  if (normalized === "WCO_FREE") return EntityFormCategory.WCO_FREE;
