...
//       // ---- POLYGON / RECTANGLE / CIRCLE / ELLIPSE – כולם fill ----
//       case 'polygon':
//       case 'rectangle':
//       case 'circle':
//       case 'ellipse':
...
//     const polygonCoords = validCoords.map(coord => [coord.lng, coord.lat]);
//     return {
//       type: 'Feature',
//       geometry: {
//         type: 'Polygon',
...
//   /** Center of polygon/circle/ellipse/rectangle/sector for label placement. Returns [lng, lat] or null. */
//   private getEntityCenter(entity: any): [number, number] | null {
//     const fillTypes = ['polygon', 'rectangle', 'circle', 'ellipse', 'sector'];
//     if (!fillTypes.includes(entity?.type)) return null;
...
//     const category = EntityCategoryEnum[entity?.category];
...
//     if (typeof category === 'string' && category.trim()) return String(category).toUpperCase();
//     if (typeof name === 'string' && name.trim() && /^[\x00-\x7F]*$/.test(name)) return name.trim();
//     return typeLabel;
//   }

//   private addEntityLabelLayer(map: maplibregl.Map, entity: any, _fillLayerId: string): void {
//     const fillTypes = ['polygon', 'rectangle', 'circle', 'ellipse', 'sector'];
//     if (!fillTypes.includes(entity?.type)) {
//       console.log('[EntityLabel] לא תווית – סוג לא fill:', entity?.id, entity?.type);
//       return;
//     }
//     const center = this.getEntityCenter(entity);
//     if (!center) {
...
//         (entity as any).color ||
//         style.fillColor ||
//         (entity.type === 'sector' && (entity as any).name === 'TABOOZONE' ? '#FFB300' : undefined) ||
//         '#3b82f6';
...
export class MapEntityManager {
  ...
  /** טקסט לתווית על המפה: שם + קטגוריה (שם enum, לא מספר) */
  private getEntityLabelForMap(entity: any): string {
    ...
    try {
      ...
      if (entity.type === 'marker') {
        ...
        return {
          ...
          'circle-stroke-color': style.strokeColor || '#1e40af',
          'circle-stroke-width': style.strokeWidth || 2,
          /** כמו fill-opacity: השדה ב-store הוא יחס אטימות (0–1), לא "כמות שקיפות" */
      ...
      if (entity.type === 'line') {
        ...
        return {
          'line-color': entityColor,
          'line-width': style.strokeWidth || (entity as any).width || 2,
          /** תואם fill-opacity / ערך ברירת מחדל מ-normalizeRawEntityToStore (1 = אטום) */
