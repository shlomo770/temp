...
export type SaveEntityCategory = "WCO_FREE" | "WCO_HOLD" | "FIZ" | "FREE";
...
export const toServerEntityCategory = (category: string): SaveEntityCategory => {
  ...
  if (normalized === "WCO_FREE") return "WCO_FREE";
  if (normalized === "WCO_HOLD") return "WCO_HOLD";
  if (normalized === "FIZ") return "FIZ";
  return "FREE";
};

export const toLocalEntityCategory = (category: string): string => {
  const normalized = String(category || "").trim().toUpperCase().replace(/\s+/g, "_");
  if (normalized === "WCO_FREE") return "WCO FREE";
  if (normalized === "WCO_HOLD") return "WCO HOLD";
  if (normalized === "FIZ") return "FIZ";
  return "FREE";
};
