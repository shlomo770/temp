...
const EntityCreationPanel: FC<EntityCreationPanelProps> = ({ isOpen, onClose, presetCategory }) => {
  ...
  const canSelectEllipse = canSelectCirclePolygonEllipse && creationCategory !== 'FIZ';
  const canSelectPolyline = Boolean(creationName.trim()) && creationCategory === 'FREE';
  ...
  const handleClose = () => {
    dispatch(setCreationForm({ name: '', category: 'FREE', height: 0 }));
  ...
  const handleCreateEntity = (type: 'circle' | 'polygon' | 'line' | 'ellipse' | 'sector') => {
    ...
    if (type === 'ellipse' && creationCategory === 'FIZ') {
      return;
    }
    if (type === 'line') {
      if (creationCategory !== 'FREE') return;
      dispatch(setCreationForm({ name: creationName, category: 'FREE', height: creationHeight }));
    }
              ...
              <div className="mb-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    title={!canSelectEllipse ? (creationCategory === 'FIZ' ? 'בקטגוריה FIZ לא ניתן ליצור Ellipse' : 'נא להזין שם ולבחור קטגוריה') : undefined}>
                    <FaEllipsisH size={22} />
